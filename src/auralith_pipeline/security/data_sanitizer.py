...
_SANITIZE_PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    ...
    ("gcp_service_account",
     re.compile(r'"private_key"\s*:\s*"-----BEGIN (?:RSA )?PRIVATE KEY-----[^"]*-----END (?:RSA )?PRIVATE KEY-----"', re.DOTALL),
    ...
    ("db_connection_string",
     re.compile(
         r"(?:mongodb|mysql|postgres|postgresql|redis|amqp|mssql)"
         r"(?:\+\w+)?://[^\s'\"]+",
         re.IGNORECASE,
     ),
     "[DB_CONNECTION_STRING]"),
...
class DataSanitizer:
        ...
        if extra_patterns:
            for name, regex, replacement in extra_patterns:
                self._patterns.append(
                    (name, re.compile(regex, re.IGNORECASE), replacement)
                )
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            **self.stats,
            "patterns_triggered": {
                k: v for k, v in self.pattern_stats.items() if v > 0
            },
        }

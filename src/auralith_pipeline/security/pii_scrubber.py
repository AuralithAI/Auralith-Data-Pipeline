...
_p(PIICategory.NATIONAL_ID_GENERIC,
   r"\b(?:ID|id|Id)[-:\s]?\d{6,12}\b")

# ── Financial ──
# Credit cards (Visa, MC, Amex, Discover etc.)
_p(PIICategory.CREDIT_CARD,
...
_p(PIICategory.SWIFT_BIC,
   r"\b(?:SWIFT|BIC|swift|bic)[:\s]+[A-Z]{6}[A-Z0-9]{2}(?:[A-Z0-9]{3})?\b")
# Crypto wallets (BTC, ETH)
_p(PIICategory.CRYPTO_WALLET,
   r"\b(?:bc1|[13])[a-zA-HJ-NP-Z0-9]{25,39}\b")
_p(PIICategory.CRYPTO_WALLET,
   r"\b0x[a-fA-F0-9]{40}\b")

# ── IP addresses ──
_p(PIICategory.IP_ADDRESS,
...
# ── URL credentials ──
_p(PIICategory.URL_CREDENTIALS,
   r"https?://[^:]+:[^@]+@")

# ── MAC address ──
_p(PIICategory.MAC_ADDRESS,
   r"\b(?:[0-9A-Fa-f]{2}[:-]){5}[0-9A-Fa-f]{2}\b")

# ── Credentials / secrets ──
_p(PIICategory.API_KEY,
   r"\b(?:api[_-]?key|apikey|token|secret|bearer)\s*[:=]\s*['\"]?[A-Za-z0-9_\-]{16,}['\"]?",
   re.IGNORECASE)
_p(PIICategory.AWS_KEY,
   r"\bAKIA[0-9A-Z]{16}\b")
_p(PIICategory.AWS_KEY,
   r"(?:aws_secret_access_key|AWS_SECRET_ACCESS_KEY)\s*[:=]\s*['\"]?[A-Za-z0-9/+=]{40}['\"]?")
_p(PIICategory.PRIVATE_KEY,
   r"-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----.*?-----END (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----",
   re.DOTALL)
_p(PIICategory.JWT_TOKEN,
   r"\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\b")
_p(PIICategory.PASSWORD,
   r"(?:password|passwd|pwd)\s*[:=]\s*['\"]?[^\s'\"]{4,}['\"]?",
   re.IGNORECASE)

# ── GPS coordinates ──
_p(PIICategory.GPS_COORDINATES,
   r"\b[-+]?(?:[1-8]?\d(?:\.\d{4,})?|90(?:\.0+)?)\s*,\s*[-+]?(?:1[0-7]\d|0?\d{1,2})(?:\.\d{4,})?\b")

# ── Date of birth (various formats) ──
_p(PIICategory.DATE_OF_BIRTH,
